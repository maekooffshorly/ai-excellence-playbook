---
name: module-builder
description: Build ACF block modules for the WordPress Boilerplate. Use this skill whenever the user wants to create a WordPress block, module, ACF block, Gutenberg block, hero section, card grid, CTA, banner, testimonial, or any reusable content component for a WordPress project. Trigger even when the user just says "build a block", "new module", or describes a layout component they want in WordPress — they don't need to say "ACF" or "boilerplate" explicitly. Also use when converting a Figma design into a WordPress block. Also trigger when the user asks how something works, what a convention is, or how to do something related to this module system — including questions about block registration, field group setup, ACF field structure, test page creation, Figma-to-module workflow, UIkit conventions, module naming, how existing modules are structured, or how to read and analyze existing module code.
---
